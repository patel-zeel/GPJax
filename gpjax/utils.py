...
def dict_array_coercion(params: Dict) -> Tuple[Callable, Callable]:
    ...
    Args:
        params (Dict): The dictionary of parameters that we would like to map into an array.

    Returns:
        Tuple[Callable, Callable]: A pair of functions, the first of which maps a dictionary to an array, and the second of which maps an array to a dictionary. The remapped dictionary is equal in structure to the original dictionary.
    """
