...
def test_gram(kern, dim):
    ...
    for fn in [kern.gram, kern.diagonal]:
...
def test_cross_covariance(kern, n1, n2):
    ...
    params, _, _, _ = initialise(kern)
    kernel_matrix = kern.cross_covariance(kern, x1, x2, params)
...
@pytest.mark.parametrize("kern", [RBF(), Matern12(), Matern32(), Matern52(), White()])
@pytest.mark.parametrize("dim", [1, 2, 5])
...
def test_active_dim(kernel):
    ...
    for dp in dim_pairs:
        ...
        k1 = kernel.gram(ad_kern, X, ad_kern.params)
        k2 = kernel.gram(manual_kern, Xslice, manual_kern.params)
        assert jnp.all(k1 == k2)


@pytest.mark.parametrize("combination_type", [SumKernel, ProductKernel])
@pytest.mark.parametrize(
    "kernel", [RBF, Matern12, Matern32, Matern52, Polynomial, White]
)
...
def test_combination_kernel(combination_type, kernel, n_kerns):
    ...
    x = jnp.linspace(0.0, 1.0, num=n).reshape(-1, 1)
    Kff = kernel.gram(c_kernel, x, c_kernel.params)
    assert Kff.shape[0] == Kff.shape[1]
    assert Kff.shape[1] == n


@pytest.mark.parametrize(
    "k1", [RBF(), Matern12(), Matern32(), Matern52(), Polynomial(), White()]
)
@pytest.mark.parametrize(
    "k2", [RBF(), Matern12(), Matern32(), Matern52(), Polynomial(), White()]
)
def test_sum_kern_value(k1, k2):
    n = 10
    sum_kernel = SumKernel(kernel_set=[k1, k2])
    x = jnp.linspace(0.0, 1.0, num=n).reshape(-1, 1)
    Kff = sum_kernel.gram(sum_kernel, x, sum_kernel.params)
    Kff_manual = k1.gram(k1, x, k1.params) + k2.gram(k2, x, k2.params)
...
def test_prod_kern_value(k1, k2):
    ...
    x = jnp.linspace(0.0, 1.0, num=n).reshape(-1, 1)
    Kff = prod_kernel.gram(prod_kernel, x, prod_kernel.params)
    Kff_manual = k1.gram(k1, x, k1.params) * k2.gram(k2, x, k2.params)
    assert jnp.all(Kff == Kff_manual)
...
def test_graph_kernel():
    ...
    Kxx = kern.gram(kern, x, kern.params)
